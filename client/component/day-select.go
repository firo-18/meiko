		...
		if i.Message.Interaction.User.String() != i.Member.User.String() {
			...
			err = s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
				...
				Data: &discordgo.InteractionResponseData{
					Embeds:     scheduleEmbeds(s, room, filler.Offset, d),
					Components: scheduleComponent(i, room, filler, d),
				},
			})
			if err != nil {
				event.ErrExit(err)
...
func scheduleEmbeds(s *discordgo.Session, room *schema.Room, offset, d int) []*discordgo.MessageEmbed {
...
func scheduleEmbedFields(room *schema.Room, d, offset int) []*discordgo.MessageEmbedField {
	...
	for h := d * 24; h < (d+1)*24 && h < len(room.Schedule); h++ {
		...
		for j, v := range room.Schedule[h] {
			filler := event.FillerList[v]
			fillers[j] = filler.User.Username
		}
...
func scheduleComponent(i *discordgo.InteractionCreate, room *schema.Room, filler *schema.Filler, d int) []discordgo.MessageComponent {
...
func scheduleComponentMenuOption(i *discordgo.InteractionCreate, filler *schema.Filler, room *schema.Room, d int) []discordgo.SelectMenuOption {
	...
	for h := d * 24; h < (d+1)*24 && h < len(room.Schedule); h++ {
		...
		switch i.Message.Interaction.Name {
		case "schedule":
			_, shift := event.HasShift(room, filler.User.ID, h)
